...
class SelfAttentionLanguageModel(nn.Module):
    """Bigram Language model based on self-attention.
    
    The model learns to predict next token using self-attention.
    """
    ...
    def forward(self, idx, targets=None):
        ...
        logits = self.lm_head(x)  # logits: outputs of the linear head (B,T,vocab_size)
        if targets is None:
            loss = None
        else:
            B, T, C = logits.shape
